...
def _emit_python_tier_fallback_warning() -> None:
    ...
    The warning folds in the loader diagnostic trail when available.
    """
    if _USING_C_TIER:
        return
    loader_trail = ""
...
def patch_cuda_graph() -> None:
    """Wrap CUDAGraph.capture_begin/capture_end/replay.

    replay is critical: replayed kernels run as one hipGraphLaunch with
    no per-op records.
    """
    if cuda_mod is None:
        return
...
def warn_dispatcher_failure_once(phase: str, error: Exception) -> None:
    """Emit one warning per (thread, phase); dispatcher callbacks must not raise."""
