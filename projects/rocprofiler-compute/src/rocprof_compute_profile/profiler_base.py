...
_SUPPORTED_FRAMEWORKS: tuple[str, ...] = ("torch", "triton")

# Each CLI flag selects exactly the backends it names; backends install
# independently. Users wanting every backend can set
# ROCPROFCOMPUTE_ROCTX_FRAMEWORKS=api (alias resolved by inject_roctx).
...
def _export_selected_frameworks(frameworks: set[str]) -> None:
    """Publish frameworks via the workload-facing environment variable."""
    if frameworks:
        os.environ[_FRAMEWORK_ENV_VAR] = ",".join(sorted(frameworks))
...
def _prepare_api_trace_injection(
    ...
    Modifies remaining in place. The rewrite depends on the workload type:
      1. Python interpreter — insert the entry point before the user script.
      2. Direct .py script  — prepend sys.executable and the entry point.
      3. Other executables  — leave the command unchanged and emit a warning.
    """
    inject_script = (
        Path(__file__).parent.parent / "utils" / "inject_roctx" / "__main__.py"
    )
    if not inject_script.exists():
        console_error(
            f"Cannot find inject_roctx package at {inject_script}. "
    ...
    def sanitize(self) -> None:
        ...
        if args.remaining:
            ...
            if selected_frameworks:
                ...
                _export_selected_frameworks(selected_frameworks)
