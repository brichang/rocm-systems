...
hipError_t GraphExec::Run(hip::Stream* launch_stream) {
  ...
  if (!event.setCallback(CL_COMPLETE, GraphExec::OnLaunchComplete, cleanup, kBlocking)) {
    ...
    // its signals). Drain that work, then return the borrowed pooled signals to
    // the pool instead of leaking them and permanently shrinking it.
    launch_stream->finish();
    if (signalManager_ != nullptr && !cleanup->signal_set.empty()) {
      signalManager_->ReleaseSet(cleanup->device, cleanup->signal_set);
    }
    delete cleanup;
    this->release();
