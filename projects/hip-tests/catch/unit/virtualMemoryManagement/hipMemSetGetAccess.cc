 ...
 * Test Description
 * ------------------------
 *    - Gating test (validation #7 of hipMemMap refactor): cross-device access
 * enumeration. Allocate physical memory on device 0 and map it. Grant access
 ...
 *
 * We poll the public API failure surface; if no public-API failure can be
 * observed (e.g., on platforms where the runtime silently routes through the
 * XGMI fabric without enforcement), the test logs a WARN. The point of this
 * test is to verify that the design's access-device enumeration on unmap
 * stays consistent with the access model the user requested.
 * ------------------------
 *    - unit/virtualMemoryManagement/hipMemSetGetAccess.cc
 * Test requirements
 * ------------------------
 ...
 * Test Description
 * ------------------------
 *    - Gating test (validation #7 of hipMemMap refactor): revoke access.
 * Allocate physical memory on dev0, map it, then grant device 1 RW access
 * (hipMemAccessFlagsProtReadWrite). Verify a kernel on device 1 works.
 * Then call hipMemSetAccess(dev1, hipMemAccessFlagsProtNone) to revoke
 * access. Attempt the same kernel on device 1 again and expect a documented
 * failure surface, or document the silent-routing gap.
 * ------------------------
 *    - unit/virtualMemoryManagement/hipMemSetGetAccess.cc
 * Test requirements
 * ------------------------
 *    - HIP_VERSION >= 6.1, requires >= 2 GPUs
 */
HIP_TEST_CASE(Unit_hipMemSetAccess_RevokedAccessFaults) {
  int devicecount = 0;
  HIP_CHECK(hipGetDeviceCount(&devicecount));
  if (devicecount < 2) {
    HIP_SKIP_TEST(HipTest::SkipReason::kFewerThanTwoGpus);
  ...
  if (!canAccessPeer) {
    WARN("Device 1 cannot access device 0 - revoke test cannot exercise positive path.");
    return;
  }
  ...
  // Grant access to both dev0 and dev1.
  hipMemAccessDesc accDesc[2]{};
  accDesc[0].location.type = hipMemLocationTypeDevice;
  accDesc[0].location.id = dev0;
  accDesc[0].flags = hipMemAccessFlagsProtReadWrite;
  accDesc[1].location.type = hipMemLocationTypeDevice;
  accDesc[1].location.id = dev1;
  accDesc[1].flags = hipMemAccessFlagsProtReadWrite;
  HIP_CHECK(hipMemSetAccess(ptrA, size_mem, accDesc, 2));

  // First, exercise from device 1 successfully.
  HIP_CHECK(hipSetDevice(1));
  hipStream_t s1 = nullptr;
  HIP_CHECK(hipStreamCreate(&s1));
  (void)hipGetLastError();
  ...
  // Now revoke device 1's access.
  hipMemAccessDesc revoke{};
  revoke.location.type = hipMemLocationTypeDevice;
  revoke.location.id = dev1;
  revoke.flags = hipMemAccessFlagsProtNone;
  HIP_CHECK(hipMemSetAccess(ptrA, size_mem, &revoke, 1));

  // Sanity check that GetAccess reflects revocation.
  unsigned long long flags = 0;  // NOLINT
  hipMemLocation location{};
  location.type = hipMemLocationTypeDevice;
  location.id = dev1;
  ...
  // Retry the kernel on device 1. Expect a documented failure surface.
  (void)hipGetLastError();
  hipLaunchKernelGGL(touch_kernel, dim3((N + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK),
                     dim3(THREADS_PER_BLOCK), 0, s1, reinterpret_cast<int*>(ptrA), N);
  hipError_t launch_err = hipGetLastError();
  ...
  if (!any_failure) {
    WARN(
        "Revoked-access kernel did not surface a public-API error on this "
        "platform; access enforcement may be performed silently. Documenting "
        "gap.");
  } else {
    INFO("launch_err=" << launch_err << " sync_err=" << sync_err << " post_err=" << post_err
                       << " destroy_err=" << destroy_err << " setdev_err=" << setdev_err
                       << " unmap_err=" << unmap_err << " free_err=" << free_err);
    SUCCEED("Use after access revoke surfaced a documented failure.");
  }
}
