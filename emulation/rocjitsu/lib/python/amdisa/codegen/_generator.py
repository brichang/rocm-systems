...
class CodeGenerator:
    ...
    def _emit_gfx1250_matrix_fmt_helpers() -> str:
        """Emit C++ helpers for gfx1250 WMMA source-format operand sizing."""
        return (
            'namespace {\n'
        ...
        if cls in _SEMA_CLASSES:
            ...
            if sema_block is not None and not sema_block.is_empty:
                ...
                is_true16_vop3 = (
                    ...
                    and is_vop3
                    and dst_operands
                    and any(opnd.is_output and opnd.size == 16 for opnd in dst_operands)
                )
                if is_true16_vop3:
        ...
        for enc in self.isa_spec.inst_encodings:
            ...
            if all_insts and not profile.is_alt_encoding(enc.enc_name):
                ...
                for inst in all_insts:
                    ...
                    _lit_enc = inst_enc_obj if inst_enc_obj is not None else enc
                    _lit_info = _LITERAL_ENCODING_OPERANDS.get(
                        _lit_enc.enc_name.upper()
                    )
                    ...
                    if _lit_info:
                        ...
                        for opnd in inst.operands:
                            ...
                            if (
                                opnd.operand_type == 'OPR_SIMM32'
                                and opnd.name not in enc_field_names
                            ):
                                ctor_body_parts.append(
                ...
                if any(
                    self._gfx1250_f8f6f4_wmma_shape(i) is not None for i in all_insts
                ):
