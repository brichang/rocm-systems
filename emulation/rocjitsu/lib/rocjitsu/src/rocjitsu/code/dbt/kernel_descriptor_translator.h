...
struct KdTranslation {
  ...
  ///
  /// @details On CDNA, COMPUTE_PGM_RSRC1 reflects .amdhsa_next_free_vgpr after
  /// descriptor granularity rounding. ACCUM_OFFSET names a separate AccVGPR
  /// window and must not be subtracted from this value, because semantic
  /// scratch allocation uses this as the lower bound for ordinary VGPR reuse.
  uint32_t guest_vgpr_count = 0;

  /// @brief Source descriptor VGPR allocation/count field after rounding.
  ///
  /// @details This is decoded from COMPUTE_PGM_RSRC1. It mirrors
  /// @c guest_vgpr_count for CDNA ordinary scratch reasoning and is kept as a
  /// separate field for diagnostics and target allocation reporting.
