...
private:
  ...
  /// @param cave_end Current end of the kernel-local cave; advanced when the
  ///                 replacement is emitted out-of-line.
  ...
  [[nodiscard]] bool apply_semantic(const struct SemanticReplacement &repl,
                                    std::vector<uint8_t> &text, uint64_t &cave_end);
  ...
  /// @param cave_end   Current end of the kernel-local cave; advanced if the
  ///                   translated encoding must be emitted out-of-line.
