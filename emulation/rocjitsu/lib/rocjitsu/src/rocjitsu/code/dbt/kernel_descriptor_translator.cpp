...
translate_one_descriptor(rj_code_arch_t guest_arch, rj_code_arch_t host_arch,
  ...
  if (arch_has_accvgpr(guest_arch) && result.accvgpr_base != 0 &&
    ...
    // CDNA assembler metadata reports ordinary VGPR pressure through
    // .amdhsa_next_free_vgpr, which is what COMPUTE_PGM_RSRC1 encodes after
    // descriptor granularity rounding. ACCUM_OFFSET names a separate AccVGPR
    // window that may start below that ordinary VGPR floor, so subtracting the
    // accumulator window here would make semantic scratch allocation reuse
    // ordinary registers that the guest compiler reserved.
  ...
  // count separate for target reporting and descriptor encoding. On CDNA,
  // COMPUTE_PGM_RSRC1 still provides the ordinary VGPR floor used by liveness
  // and semantic scratch allocation; ACCUM_OFFSET is an additional AccVGPR
  // window that can overlap that ordinary floor in the descriptor namespace.
  //
