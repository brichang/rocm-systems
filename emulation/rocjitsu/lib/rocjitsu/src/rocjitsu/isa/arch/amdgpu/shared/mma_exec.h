...
// gfx1250 WMMA is wave32-only. These formulas mirror the lane layouts in
// LLVM's gfx1250 *_w32 instruction definitions.
...
inline uint32_t read_swmmac_index_word(amdgpu::ComputeUnitCore &cu, uint32_t index_base,
                                       uint32_t lane, uint32_t index_entries, uint32_t index_key,
                                       uint32_t k_group) {
  // FMT_WMMA_INDEX_SET2 is a 64-bit operand, but each 32-bit VGPR word carries
  // one reduction-group window of 16 packed 2-bit entries.
  switch (index_entries) {
  case 8:
  case 16:
    return static_cast<uint32_t>(
        read_swmmac_index_set(cu, index_base, lane, index_entries, index_key));
  case 32:
    if (index_key & 0x1u)
      return cu.read_vgpr(index_base + 1, lane);
    return cu.read_vgpr(index_base + (k_group & 0x1u), lane);
  default:
    throw util::UnimplementedInst("unsupported SWMMAC sparse index width");
  }
}
...
inline uint32_t swmmac_dense_k_from_word(uint32_t index_word, uint32_t compressed_k,
                                         uint32_t local_compressed_k) {
  const uint32_t sparse_index = (index_word >> (2 * (local_compressed_k & 0xFu))) & 0x3u;
  return (compressed_k / 2) * 4 + sparse_index;
}
...
void exec_swmmac_f32_mixed(amdgpu::ComputeUnitCore &cu, uint32_t M, uint32_t N, uint32_t K,
  ...
  const uint32_t compressed_elems_per_lane = compressed_k / (WMMA_WAVE32 / M);
  for (uint32_t row = 0; row < M; ++row) {
    for (uint32_t col = 0; col < N; ++col) {
      auto out = wmma_output_loc_32(M, N, row, col);
      float acc = (const_acc != ACC_FROM_VGPR)
      ...
      for (uint32_t ck = 0; ck < compressed_k; ++ck) {
        auto al = wmma_input_loc(M, compressed_k, row, ck, a_bits);
        const uint32_t local_ck = ck % compressed_elems_per_lane;
        const uint32_t k_group = ck / compressed_elems_per_lane;
        const uint32_t index_word =
            read_swmmac_index_word(cu, index_base, al.lane, index_entries, index_key, k_group);
        const uint32_t dense_k = swmmac_dense_k_from_word(index_word, ck, local_ck);
...
void exec_swmmac_packed16(amdgpu::ComputeUnitCore &cu, uint32_t M, uint32_t N, uint32_t K,
  ...
  for (uint32_t row = 0; row < M; ++row) {
    for (uint32_t col = 0; col < N; ++col) {
      ...
      for (uint32_t ck = 0; ck < compressed_k; ++ck) {
        auto al = wmma_input_loc(M, compressed_k, row, ck, in_bits);
        const uint32_t local_ck = ck % compressed_elems_per_lane;
        const uint32_t k_group = ck / compressed_elems_per_lane;
        const uint32_t index_word =
            read_swmmac_index_word(cu, index_base, al.lane, index_entries, index_key, k_group);
