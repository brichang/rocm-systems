...
void VCmpLtF16Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    if ((util::f16_to_f32(static_cast<uint16_t>(src0.read_lane(wf, lane))) <
...
void VCmpOF16Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    if ((!std::isnan(util::f16_to_f32(static_cast<uint16_t>(src0.read_lane(wf, lane)))) &&
         !std::isnan(util::f16_to_f32(static_cast<uint16_t>(vsrc1.read_lane(wf, lane))))))
      vcc |= (1ULL << lane);
    else
      vcc &= ~(1ULL << lane);
  }
...
void VCmpLtF32Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    if ((std::bit_cast<float>(src0.read_lane(wf, lane)) <
...
void VCmpOF32Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    if ((!std::isnan(std::bit_cast<float>(src0.read_lane(wf, lane))) &&
         !std::isnan(std::bit_cast<float>(vsrc1.read_lane(wf, lane)))))
      vcc |= (1ULL << lane);
    else
      vcc &= ~(1ULL << lane);
  }
...
void VCmpLtF64Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    if ((std::bit_cast<double>(src0.read_lane64(wf, lane)) <
...
void VCmpOF64Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    if ((!std::isnan(std::bit_cast<double>(src0.read_lane64(wf, lane))) &&
         !std::isnan(std::bit_cast<double>(vsrc1.read_lane64(wf, lane)))))
      vcc |= (1ULL << lane);
    else
      vcc &= ~(1ULL << lane);
  }
...
void VCmpLtI16Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    if ((static_cast<int16_t>(src0.read_lane(wf, lane)) <
         static_cast<int16_t>(vsrc1.read_lane(wf, lane))))
      vcc |= (1ULL << lane);
    else
      vcc &= ~(1ULL << lane);
  }
...
void VCmpLtU16Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    if ((static_cast<uint16_t>(src0.read_lane(wf, lane)) <
         static_cast<uint16_t>(vsrc1.read_lane(wf, lane))))
      vcc |= (1ULL << lane);
    else
      vcc &= ~(1ULL << lane);
  }
...
void VCmpLtU32Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    if ((src0.read_lane(wf, lane) < vsrc1.read_lane(wf, lane)))
      vcc |= (1ULL << lane);
    else
      vcc &= ~(1ULL << lane);
  }
...
void VCmpLtI64Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    if ((static_cast<int64_t>(src0.read_lane64(wf, lane)) <
         static_cast<int64_t>(vsrc1.read_lane64(wf, lane))))
      vcc |= (1ULL << lane);
    else
      vcc &= ~(1ULL << lane);
  }
...
void VCmpClassF16Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    if ([&]() -> bool {
          ...
          uint32_t mask = static_cast<uint32_t>(vsrc1.read_lane(wf, lane));
          bool match = false;
...
void VCmpClassF64Vopc::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    if ([&]() -> bool {
          ...
          uint32_t mask = static_cast<uint32_t>(vsrc1.read_lane64(wf, lane));
          bool match = false;
