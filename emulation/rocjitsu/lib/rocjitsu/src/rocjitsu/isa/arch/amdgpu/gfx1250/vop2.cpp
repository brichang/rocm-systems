...
void VCndmaskB32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(
        wf, lane, ((wf.vcc() >> lane) & 1) ? vsrc1.read_lane(wf, lane) : src0.read_lane(wf, lane));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
...
void VAddF32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane,
                    std::bit_cast<uint32_t>((std::bit_cast<float>(src0.read_lane(wf, lane)) +
...
void VSubrevF32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane,
                    std::bit_cast<uint32_t>((std::bit_cast<float>(vsrc1.read_lane(wf, lane)) -
...
void VMulDx9ZeroF32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane, std::bit_cast<uint32_t>([&]() {
                      auto a = std::bit_cast<float>(src0.read_lane(wf, lane));
                      auto b = std::bit_cast<float>(vsrc1.read_lane(wf, lane));
...
void VMulI32I24Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane, [&]() {
      ...
      auto b = static_cast<int32_t>(vsrc1.read_lane(wf, lane) << 8) >> 8;
...
void VMulHiI32I24Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane, [&]() {
      ...
      auto b = static_cast<int64_t>(static_cast<int32_t>(vsrc1.read_lane(wf, lane) << 8) >> 8);
...
void VMulU32U24Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(
        wf, lane,
        ((src0.read_lane(wf, lane) & 0x00FFFFFFu) * (vsrc1.read_lane(wf, lane) & 0x00FFFFFFu)));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
    uint64_t dpp_write_mask = 0;
    for (uint32_t ln = 0; ln < wf.wf_size(); ++ln) {
...
void VMulHiU32U24Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane, [&]() {
      ...
      auto b = static_cast<uint64_t>(vsrc1.read_lane(wf, lane) & 0x00FFFFFFu);
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane,
                    std::min(static_cast<int32_t>(src0.read_lane(wf, lane)),
                             static_cast<int32_t>(vsrc1.read_lane(wf, lane))));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
    uint64_t dpp_write_mask = 0;
    for (uint32_t ln = 0; ln < wf.wf_size(); ++ln) {
...
void VMinU32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane, std::min(src0.read_lane(wf, lane), vsrc1.read_lane(wf, lane)));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
    uint64_t dpp_write_mask = 0;
    for (uint32_t ln = 0; ln < wf.wf_size(); ++ln) {
...
void VMaxU32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane, std::max(src0.read_lane(wf, lane), vsrc1.read_lane(wf, lane)));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
    uint64_t dpp_write_mask = 0;
    for (uint32_t ln = 0; ln < wf.wf_size(); ++ln) {
...
void VLshlrevB32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane, (vsrc1.read_lane(wf, lane) << (src0.read_lane(wf, lane) & 31u)));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
    uint64_t dpp_write_mask = 0;
    for (uint32_t ln = 0; ln < wf.wf_size(); ++ln) {
...
void VAshrrevI32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane, [&]() {
      auto v = static_cast<int32_t>(static_cast<int32_t>(vsrc1.read_lane(wf, lane)));
...
void VAndB32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane, (src0.read_lane(wf, lane) & vsrc1.read_lane(wf, lane)));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
    uint64_t dpp_write_mask = 0;
    for (uint32_t ln = 0; ln < wf.wf_size(); ++ln) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane, [&]() {
      uint64_t w = static_cast<uint64_t>(src0.read_lane(wf, lane)) +
                   static_cast<uint64_t>(vsrc1.read_lane(wf, lane)) +
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    ...
    vdst.write_lane(wf, lane, [&]() {
      uint64_t a = static_cast<uint64_t>(src0.read_lane(wf, lane)),
               b = static_cast<uint64_t>(vsrc1.read_lane(wf, lane)),
...
void VSubrevCoCiU32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane, [&]() {
      uint64_t a = static_cast<uint64_t>(vsrc1.read_lane(wf, lane)),
...
void VSubrevNcU32Vop2::execute_impl(amdgpu::Wavefront &wf) {
  ...
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
    vdst.write_lane(wf, lane, (vsrc1.read_lane(wf, lane) - src0.read_lane(wf, lane)));
  }
  if (inst_.src0 == amdgpu::SRC_DPP) {
    uint64_t dpp_write_mask = 0;
    for (uint32_t ln = 0; ln < wf.wf_size(); ++ln) {
